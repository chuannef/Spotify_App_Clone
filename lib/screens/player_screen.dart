  ...
  Future<void> _loadAndPlayTrack() async {
    try {      
      ...
      if (widget.albumTitle.toLowerCase().contains('bảo tàng của nuối tiếc')) {
        ...
        int index = (widget.currentTrack! - 1) % trackFiles.length;
        audioPath = 'assets/sound/Lặng/${trackFiles[index]}';
        print('Lặng album detected. Track index: $index, Selected file: ${trackFiles[index]}');
      ...
      if (mounted) {
        try {
          // For web, we need to handle audio playback differently
          if (kIsWeb) {
            // For web, try to use a different approach
            print('Running on web platform');
            final webPath = audioPath.replaceFirst('assets/', '');
            print('Web audio path: $webPath');
            
            await audioPlayer.setSource(AssetSource(webPath));
            print('Audio source set successfully with AssetSource');
          } else {
            // For mobile platforms
            await audioPlayer.setSource(AssetSource(audioPath.replaceFirst('assets/', '')));
            print('Audio source set for mobile');
          }
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Error playing track: $audioError'),
